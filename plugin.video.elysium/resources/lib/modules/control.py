...
def artPath():
    theme = appearance()
    if theme in ['-', '']: return
    elif condVisibility('System.HasAddon(script.elysium.artwork)'):
        return os.path.join(xbmcaddon.Addon('script.elysium.artwork').getAddonInfo('path'), 'resources', 'media', theme)
...
def artwork():
    execute('RunPlugin(plugin://script.elysium.artwork)')
