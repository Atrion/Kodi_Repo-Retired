            ...
            if "'tvdb': " in str(item) or "'tvdb_id': " in str(item):
                ...
                if not tmdb or tmdb == None or tmdb =="None":
                    ...
                    if not imdb and not tvdb_id:
                        return dialogs.notify(msg="No id found for item", title='', delay=3000, image=get_icon_path("chappaai"))
...
def lists_trakt_add_my_lists_to_library():
    ...
    import xbmcgui
    lists = trakt.trakt_get_lists()
    dialogs.notify(msg='Adding ' + str(len(lists)) + " lists", title='to Kodi library', delay=3000, image=get_icon_path("chappaai"))
    for list in lists:
        user = list["user"]["username"]
        slug = list["ids"]["slug"]
        items = lists_trakt_show_list(user, slug)
