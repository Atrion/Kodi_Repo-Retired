...
def speedtest(list = False , mini = None , server = None , share = True , simple = False , src = None , timeout = 10 , units = ( 'bit' , 8 ) , version = False):
	...
	dp.create("%s: [COLOR %s]Speed Test[/COLOR]" % (ADDONTITLE, COLOR1), line1)
	dp.update(0)
	...
	line2 = "[COLOR %s]Retrieving speedtest.net configuration...[/COLOR]" % COLOR2
	...
	line3 = "[COLOR %s]Retrieving speedtest.net server list...[/COLOR]" % COLOR2
	dp.update(4, line1, line2, line3)

	servers = closestServers(config['client'])

	print_('Testing from %(isp)s (%(ip)s)...' % config['client'])
	line1 = '[COLOR '+COLOR2+']Testing From:[/COLOR] [COLOR '+COLOR1+']%(isp)s (%(ip)s)[/COLOR]' % config['client']
	dp.update(6, line1)

	print_('Selecting best server based on latency...')
	line2 = "[COLOR %s]Selecting best server based on latency...[/COLOR]" % COLOR2
	dp.update(8, "", line2, )
	...
	line2 = ('[COLOR '+COLOR2+']Server location: %(name)s [%(d)0.2f km]: '
			'%(latency)s ms[/COLOR]' % best).encode('utf-8', 'ignore')
	dp.update(10, "", line2)	
	...
	line2 = "[COLOR %s]Testing download speed:[/COLOR] [COLOR %s]%0.2f M%s/s[/COLOR]" % (COLOR2, COLOR1, (dlspeed / 1000 / 1000) * units[1], units[0])
	...
	while ulspeed < 1:
		...
		dp.update(65, "", "", "[COLOR "+COLOR2+"]Testing upload speed... [Attempt [/COLOR]" + str(i) + "]")
		ulspeed = uploadSpeed(best['url'], sizes, simple)
		print_()
		print_('Upload: %0.2f M%s/s' %
		   ((ulspeed / 1000 / 1000) * units[1], units[0]))
	...
	line1 = line2
	line2 = "[COLOR %s]Testing upload speed:[/COLOR] [COLOR %s]%0.2f M%s/s[/COLOR]" % (COLOR2, COLOR1, (ulspeed / 1000 / 1000) * units[1], units[0])
