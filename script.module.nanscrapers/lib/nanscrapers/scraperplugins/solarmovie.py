...
class solarmovie(Scraper):
    domains = ['http://www.solarmovie.ms']
    name = "SolarMovie"
    sources = []
    ...
    def scrape_movie(self, title, year, imdb, debrid = False):
        try:
            ...
            start_url = '%s/keywords/%s' %(self.base_link,search_id.replace(' ','%20'))  
            #print 'SolarMovie - scrape_movie - start_url:  ' + start_url           
            
            headers={'User-Agent':random_agent()}
            html = requests.get(start_url,headers=headers,timeout=5).content     
            
            match = re.compile('<span class="name"><a title="(.+?)" href="(.+?)".+?title="(.+?)"',re.DOTALL).findall(html)
            for name,item_url, year in match:
                if year in year:                                                        
    ...
    def get_source(self,item_url):
        try:
            ...
            Endlinks = re.compile('id="link_.+?target="_blank" id="(.+?)"',re.DOTALL).findall(OPEN)
