...
chappaai = xbmcaddon.Addon("plugin.video.chappaai")
...
def get_addons(type=None):
    ...
    if xbmcvfs.exists(path):
        ...
        addons = []
        ids = []
        players_path = "special://profile/addon_data/plugin.video.chappaai/players/"
        ...
        for file in files:
            ...
            try:
                ...
                if type == "all":
                    ...
                    addons.append(addon)
                    ids.append(str(meta["id"]))
                elif meta[type] and xbmc.getCondVisibility('System.HasAddon(%s)' % meta["plugin"]) and meta["plugin"] not in str(addons) and meta["plugin"] != 'plugin.video.chappaai' and meta["plugin"] != 'script.qlickplay' and meta["plugin"] != 'plugin.video.youtube' and meta["plugin"] != 'script.extendedinfo':
                    addon = []
                    addon.append(str(meta["plugin"]))
                    addon.append(str(meta["id"]))
                    addons.append(addon)
