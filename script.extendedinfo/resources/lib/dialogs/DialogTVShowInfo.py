...
def get_tvshow_window(window_type):
    ...
    class DialogTVShowInfo(DialogBaseInfo, window_type):
        ...
        def pick_and_mix(self):
            url = 'plugin://plugin.video.chappaai/tv/play/%s/1/1/%s' % (self.info.get("tvdb_id", ""), self.listitem.getProperty("qid"))
        ...
        def play_tvshow_no_resume(self):
            ...
            if self.dbid:
                ...
                if not os.path.exists(url):
                    url = "plugin://plugin.video.chappaai/tv/play/%s/1/1/%s" % (self.info.get("tvdb_id", ""), SETTING("player_main_tv"))
            else:
                url = "plugin://plugin.video.chappaai/tv/play/%s/1/1/%s" % (self.info.get("tvdb_id", ""), SETTING("player_main_tv"))
        ...
        def add_tvshow_to_library(self):
            ...
            if not os.path.exists(xbmc.translatePath("%s%s/" % (TVLibrary, tvdb_id))):
                xbmc.executebuiltin("RunPlugin(plugin://plugin.video.chappaai/tv/add_to_library/%s)" % self.info.get("tvdb_id", ""))
        ...
        @ch.click(120)
        def browse_tvshow(self):
            url = "plugin://plugin.video.chappaai/tv/tvdb/%s" % self.info.get("tvdb_id", "")
            self.close()
