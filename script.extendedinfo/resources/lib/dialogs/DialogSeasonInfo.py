    ...
    class DialogSeasonInfo(DialogBaseInfo, window_type):
        ...
        def browse_season(self):
            if self.dbid:
                ...
                url = "plugin://plugin.video.chappaai/tv/tvdb/%s/%s/" % (tvdb_id, self.info.get("season", ""))
            self.close()
        ...
        def play_season_no_resume(self):
            if self.dbid:
                ...
                if not os.path.exists(url):
                    url = "plugin://plugin.video.chappaai/tv/play/%s/%s/1/%s" % (self.info.get("tvdb_id", ""), self.info.get("season", ""), SETTING("player_main_tv"))
            else:
                url = "plugin://plugin.video.chappaai/tv/play/%s/%s/1/%s" % (self.info.get("tvdb_id", ""), self.info.get("season", ""), SETTING("player_main_tv"))
        ...
        def pick_and_mix(self):
            url = 'plugin://plugin.video.chappaai/tv/play/%s/%s/1/%s' % (self.info.get("tvdb_id", ""), self.info.get("season", ""), self.listitem.getProperty("qid"))
