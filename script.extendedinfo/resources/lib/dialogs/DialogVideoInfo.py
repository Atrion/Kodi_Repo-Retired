...
def get_movie_window(window_type):
    ...
    class DialogVideoInfo(DialogBaseInfo, window_type):
        ...
        def add_list_to_account(self):
            ...
            if selection == -1:
                ...
                for item in movies_list:
                    if item != "":
                        ids = ids + str(item[1]) + '\n'
                batch_add_file_path = "special://profile/addon_data/plugin.video.chappaai/movies_to_add.txt"
                ...
                xbmc.executebuiltin("RunPlugin(plugin://plugin.video.chappaai/movies/add_to_library/tmdb/%s)" % str(movies_list[selection][1]))
        ...
        @ch.click(120)
        def search_in_chappaai_by_title(self):
        ...
        def play_movie_no_resume(self):
            if self.dbid:
                ...
                dbid = 0
                url = "plugin://plugin.video.chappaai/movies/play/tmdb/%s/%s" % (self.info.get("id", ""), SETTING("player_main_movie"))
        ...
        def show_manage_dialog(self):
            ...
            manage_list.append([xbmc.getInfoLabel('System.AddonTitle(%s)' % 'plugin.video.chappaai') + " " + LANG(10004), 'Addon.OpenSettings(%s)' % 'plugin.video.chappaai'])
        ...
        def appintegration_movie(self):
            ...
            if xbmc.getCondVisibility("system.hasaddon(plugin.video.placenta)"):
                manage_list.append(["Search Placenta", "ActivateWindow(10025,plugin://plugin.video.placenta/?action=movieSearch&query="+title+",return)||Notification(Placenta:,"+title+",5000,special://home/addons/plugin.video.placenta/icon.png)"])
            ...
            if xbmc.getCondVisibility("system.hasaddon(plugin.video.blamo)"):
                manage_list.append(["Search Death Streams", "ActivateWindow(10025,plugin://plugin.video.blamo/?query="+title+"&section=Movies&mode=search_results,return)||Notification(Death Streams:,"+title+",5000,special://home/addons/plugin.video.blamo/icon.png)"])
        ...
        def pick_and_mix(self):
            dbid = 0
            url = "plugin://plugin.video.chappaai/movies/play/tmdb/%s/%s" % (self.info.get("id", ""), self.listitem.getProperty("qid"))
        ...
        def add_movie_to_library(self):
            MovieLibrary = chappaai.getSetting("movies_library_folder")
            imdb_id = self.info["imdb_id"]
            if not os.path.exists(xbmc.translatePath("%s%s/" % (MovieLibrary, imdb_id))):
                xbmc.executebuiltin("RunPlugin(plugin://plugin.video.chappaai/movies/add_to_library/tmdb/%s)" % self.info.get("id", ""))
