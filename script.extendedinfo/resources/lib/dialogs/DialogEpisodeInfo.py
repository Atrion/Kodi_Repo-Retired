...
def get_episode_window(window_type):
    ...
    class DialogEpisodeInfo(DialogBaseInfo, window_type):
        ...
        def play_episode_no_resume(self):
            if self.dbid:
                dbid = self.dbid
                url = "special://profile/playlists/mixed/chappaai/TVShows/%s.xsp" % self.info.get("tvdb_id", "")
                if not os.path.exists(url):
                    url = "plugin://plugin.video.chappaai/tv/play/%s/%s/%s/%s" % (self.info.get("tvdb_id", ""), self.info.get("season", ""), self.info["episode"], SETTING("player_main_tv"))
            else:
                dbid = 0
                tvdb_id = fetch(get_tvshow_ids(self.tvshow_id), "tvdb_id")
                url = "plugin://plugin.video.chappaai/tv/play/%s/%s/%s/%s" % (tvdb_id, self.info["season"], self.info["episode"], SETTING("player_main_tv"))
        ...
        def show_manage_dialog(self):
            ...
            manage_list.append([xbmc.getInfoLabel('System.AddonTitle(%s)' % 'plugin.video.chappaai') + " " + LANG(32133), 'Addon.OpenSettings(%s)' % 'plugin.video.chappaai'])
        ...
        def pick_and_mix(self):
            url = 'plugin://plugin.video.chappaai/tv/play/%s/%s/%s/%s' % (self.data.get("tvdb_id", ""), self.info.get("season", ""), self.info.get("episode", ""), self.listitem.getProperty("qid"))
