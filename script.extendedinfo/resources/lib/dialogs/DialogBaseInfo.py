            ...
            if selection == 0:
                if self.type == "TVShow":
                    if dbid:
                        ...
                        xbmc.executebuiltin("ActivateWindow(10025,plugin://plugin.video.chappaai/tv/tvdb/%s,return)" % tvdb)
                        self.close()
                else:
                    if dbid: url = "temp"
                    else:
                        dbid = 0
                        url = "plugin://plugin.video.chappaai/movies/play/tmdb/%s/%s" % (item_id, SETTING("player_main_movie"))
                    PLAYER.qlickplay(url, listitem=None, window=self, dbid=dbid)
            if selection == 1:
                if self.type == "TVShow":
                    if dbid:
                        show_path = xbmc.translatePath("special://profile/addon_data/plugin.video.chappaai/TVShows/%s" % tvdb)
                        ...
                        xbmc.executebuiltin("RunPlugin(plugin://plugin.video.chappaai/movies/add_to_library/tmdb/%s)" % item_id)
                        xbmc.executebuiltin("RunScript(script.extendedinfo,info=afteradd)")
                else:
                    if dbid:
                        movie_path = xbmc.translatePath("special://profile/addon_data/plugin.video.chappaai/Movies/%s" % imdb)
