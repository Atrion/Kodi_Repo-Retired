...
PlacentaSettings = tools.File.readNow(tools.File.joinPath(tools.System.profile(tools.Extensions.IdPlacenta), 'settings.xml'))
...
class source:
	...
	def instanceEnabled(self):
		...
		if not self.id == '':
			global PlacentaSettings
			try:
				result = tools.Settings.raw('provider.' + self.id, parameter = tools.Settings.ParameterValue, data = PlacentaSettings)
	...
	def _instancesPath(self):
		return tools.File.joinPath(tools.System.profile(), 'Scrapers', 'Placenta')
	...
	def _instancesPrepare(self):
		# Placenta's imports (from resources.lib...) clash with Gaia's imports.
		# Copy Placenta's code and change all import statements
		pathSource = tools.System.path(tools.Extensions.IdPlaScrapers)
		...
		if not tools.File.exists(fileDesitnation) or not tools.Hash.fileSha1(fileSource) == tools.Hash.fileSha1(fileDesitnation):
			...
			pathPlacenta = tools.File.joinPath(pathDestination, 'lib', 'placenta')
			tools.File.renameDirectory(pathResources, pathPlacenta)
			self._instancesRename(pathPlacenta)
		self._instancesInclude()
		return tools.File.joinPath(pathDestination, 'lib', 'placenta', 'lib', 'sources')
				...
				for package2, name2, pkg2 in pkgutil.walk_packages(path2):
					if not pkg2:
						try:
							...
							scraperNew.enabled = tools.Settings.raw('provider.' + id, parameter = tools.Settings.ParameterValue, data = PlacentaSettings)
