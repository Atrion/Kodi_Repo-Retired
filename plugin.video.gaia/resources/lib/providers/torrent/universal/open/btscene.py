...
class source:
	...
	def __init__(self):
		...
		self.priority = 0
		self.language = ['un']
		self.domains = ['bt-scene.cc']
		self.base_link = 'https://bt-scene.cc'
		self.search_link = '/results_.php?q=%s&category=%s&page=%i'
		self.category_movies = 'movies'
		self.category_tvshows = 'series'
	...
	def sources(self, url, hostDict, hostprDict):
		...
		try:
			...
			while True:
				...
				htmlTable = html.find_all('table', class_ = 'tor')[0]
				...
				for i in range(1, len(htmlRows)):
					...
					# Name
					htmlName = htmlRow.find_all('td', class_ = 'tname', recursive = False)[0].getText().strip()

					# Size
					htmlSize = htmlRow.find_all('td', class_ = 'tsize', recursive = False)[0].getText().strip()

					# Link
					htmlLink = htmlRow.find_all('td', class_ = 'tname', recursive = False)[0].find_all('a')[0]['href'].strip()
					htmlLink = self.base_link + htmlLink

					# Seeds
					htmlSeeds = int(htmlRow.find_all('td', class_ = 'tseeds', recursive = False)[0].getText().strip())
	...
	def resolve(self, url):
		...
		html = html.find_all('div', class_ = 'analytics_container')[0]
		html = html.find_all('p', recursive = False)[0]
		html = html.find_all('a', recursive = False)[0]
		return html['href']
