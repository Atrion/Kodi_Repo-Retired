	...
	def sources(self, url, hostDict, hostprDict):
		...
		try:
			...
			while True:
				...
				htmlRows = htmlTable.find_all('tr', class_ = 't-row', recursive = False) # Do not search further down the tree (just the direct children), because that will also retrieve the header row.
				for i in range(len(htmlRows)):
					try:
						...
						# Link
						htmlLink = htmlColumns[1].find_all('a')[1]['href'].strip()
						htmlLink = urlparse.urljoin(self.base_link, htmlLink)

						# Seeds
						htmlSeeds = int(re.sub('[^0-9]', '', htmlColumns[7].getText()))
