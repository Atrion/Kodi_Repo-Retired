		...
		try:
			...
			while True:
				...
				for i in range(len(htmlRows)):
					...
					# Seeds
					try: htmlSeeds = int(htmlRow.find_all('td', class_ = 'seeders')[0].getText().strip())
					...
					# Add
					sources.append({'url' : htmlLink, 'debridonly' : False, 'memberonly' : True, 'direct' : False, 'source' : 'torrent', 'language' : self.language[0], 'quality':  meta.videoQuality(), 'metadata' : meta, 'file' : htmlName})
					added = True
