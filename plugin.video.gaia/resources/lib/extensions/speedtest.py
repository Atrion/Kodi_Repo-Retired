	...
	def show(self, update = UpdateDefault):
		...
		if self.mError:
			...
			if not update == None:
				...
				if updateManual:
					tools.Settings.set(id = 'playback.manual.bandwidth.maximum', value = option)
				if updateAutomatic:
					tools.Settings.set(id = 'playback.automatic.bandwidth.maximum', value = option)
...
class SpeedTesterOffCloud(SpeedTesterDebrid):
	...
	def _validate(self):
		if not self.mLinks == None and not len(self.mLinks) == 0 and self.mLinks[0] == self.LinkServer or debrid.OffCloud().accountValid():
			return True
		else:
