...
class Api(object):
	...
	def supportCategories(self):
		return self._request(type = self.TypeSupport, action = self.ActionCategories)

	@classmethod
	def supportList(self, category = None):
		parameters = {}
		if not category == None and not category == '': parameters[self.ParameterId] = category
		return self._request(type = self.TypeSupport, action = self.ActionList, parameters = parameters)

	@classmethod
	def supportQuestion(self, id):
		parameters = {}
		parameters[self.ParameterId] = id
		return self._request(type = self.TypeSupport, action = self.ActionRetrieve, parameters = parameters)
